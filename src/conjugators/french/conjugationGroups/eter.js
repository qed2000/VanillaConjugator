...
export class Eter extends FullConjugation {
    static rule = {
        priority: 1000,
        test: (verb) => verb.endsWith('jeter') || verb.endsWith('queter') || verb.endsWith('feuilleter')
    };
    ...
    constructor(verbIndex, helper, reflexive) {
        ...
        const verbRoot = `${ this.getRoot(verb, 2) }t`; // -ett
        const verbRoot2 = this.getRoot(verb, 2); // -et
